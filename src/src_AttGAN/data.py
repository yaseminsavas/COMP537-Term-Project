...
def make_celeba_dataset(img_dir,
    ...
    img_paths = np.array([img_dir.join("/"+img_name) for img_name in img_names])
    ...
    labels = np.take(a_fin,np.array([ATT_ID[att_name] for att_name in att_names]))
    ...
    if training:
        def map_fn_(img, label2):
            img = tf.compat.v1.image.resize(img, [load_size, load_size])
            img = tf.compat.v1.image.random_flip_left_right(img)
            img = tf.compat.v1.image.random_crop(img, [crop_size, crop_size, 3])
